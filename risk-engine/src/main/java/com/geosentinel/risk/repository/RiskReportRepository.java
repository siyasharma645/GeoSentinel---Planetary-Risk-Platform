...
public interface RiskReportRepository extends JpaRepository<LocationRiskReport,UUID> {
    @Query("SELECT r FROM LocationRiskReport r WHERE ABS(r.lat - :lat) < 0.05 AND ABS(r.lon - :lon) < 0.05 AND r.expiresAt > :now ORDER BY r.generatedAt DESC LIMIT 1")
